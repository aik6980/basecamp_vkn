...
void Main_renderer::draw()
{
    ...
    case Render_mode::VerificationCompute:
        ...
        build_fullscreen_sample_passes(*m_frame_graph,
            m_frame_graph->get_or_create_resource_id("compute_output"),
            Gfx_main::resource_manager().get_texture("t_compute_output").m_image,
            "t_compute_output");
        break;
    case Render_mode::VerificationRaytrace:
        build_verification_raytrace_passes(*m_frame_graph);
        build_fullscreen_sample_passes(*m_frame_graph,
            m_frame_graph->get_or_create_resource_id("raytrace_output"),
            Gfx_main::resource_manager().get_texture("t_raytracing_output").m_image,
            "t_raytracing_output");
        break;
...
void Main_renderer::build_fullscreen_sample_passes(
    Frame_graph& frame_graph, uint32_t src_resource_id, vk::Image src_image, const std::string& src_texture_name)
{
    PassNode vis_pass;
    vis_pass.name = "visualize_" + src_texture_name;
    vis_pass.reads.push_back(ResourceUse{
        ...
        .is_image    = true,
        .image       = src_image,
        .image_range = {vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1},
    });

    auto&& render_target_image       = Gfx_main::gfx_device().offscreen_colour_image();
    const uint32_t res_render_target = frame_graph.get_or_create_resource_id("render_target");
    vis_pass.writes.push_back(ResourceUse{
        .resource_id = res_render_target,
        .is_write    = true,
        .layout      = vk::ImageLayout::eColorAttachmentOptimal,
        .access      = vk::AccessFlagBits2::eColorAttachmentWrite,
    ...
    std::string tex_name = src_texture_name; // capture by value

    vis_pass.execute = [tex_name](vk::CommandBuffer& cmd) {
        auto&& gfx_device     = Gfx_main::gfx_device();
        auto&& shader_manager = Gfx_main::shader_manager();
        // setup render pass
        auto&& render_target_image_view = gfx_device.offscreen_colour_image_view();
        ...
        const bool texture_ok = technique_instance.bind_sampled_image_by_name("ColourTex_srv", tex_name);
        const bool sampler_ok = technique_instance.bind_sampler_by_name("Linear_sam", "s_linear_wrap");
